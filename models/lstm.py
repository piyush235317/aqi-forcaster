...
class AQILSTM:
    ...
    def create_sequences(self, data):
        """
        Converts data into (Samples, TimeSteps, Features) format.
        """
        X, y = [], []
    ...
    def build_model(self, input_shape, config='A'):
        """
        Builds the LSTM model based on config.
        """
        model = Sequential()
        if config == 'A':
    ...
    def train(self, train_data):
        ...
        Expects train_data to be a numpy array or list of AQI values.
        """
    ...
    def predict(self, data):
        ...
        Expects data to be the recent history to predict from.
        """
        # Create sequences (usually for test set evaluation)
        data = np.array(data)
        
        # If data is a long sequence (test set), we create sequences
        X_test, _ = self.create_sequences(data)
        
        if len(X_test) == 0:
             # Handle case where data is just one window
             if len(data) == self.window_size:
